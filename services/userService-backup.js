...
class UserService {
  ...
  static async getUserByFitnessId(fitnessId) {
    try {
      const mongoose = require('mongoose');
      const db = mongoose.connection.db;
      const collection = db.collection('users');
      
      const user = await collection.findOne({ fitnessId: fitnessId.toUpperCase() });
      if (!user) {
        return null;
      }
      ...
      console.error('Error getting user by fitness ID:', error);
      throw error;
    }
  }
